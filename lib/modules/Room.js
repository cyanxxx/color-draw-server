...
export default class Room{
  ...
  addUser(firstTime, ctrl) {
    var data = {}
    if(this.userList.length === 0 || firstTime){
      if(!this.ownerId)this.ownerId = ctrl.user.id;
      this.userList.push(ctrl);
      var roomLists = global.roomsMap.map((el)=>{
        return el.userList.map((ctrl)=>{
          return ctrl.user.userInfo()
        })
      })
      data = {roomLists}
    }
    else if(this.status === WAITING && !this.isFull()){
      this.userList.push(ctrl);
      var user = ctrl.user
